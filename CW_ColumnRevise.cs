...
namespace Ribbon
{
    ...
    public class CW_ColumnRevise : IExternalCommand
    {
        public Result Execute(ExternalCommandData cD, ref string ms, ElementSet set)
        {
            ...
            //柱修改的设置
            XYZ point2 = new XYZ(point.X + 40 / 304.8, point.Y + 10/304.8, point.Z);

            FamilyInstance familyInstance = doc.Create.NewFamilyInstance(point2, familyInstance1.Symbol, top, familyInstance1.StructuralType);

            familyInstance.SetTopLevelExt(top.Id);//设置柱顶标高
            familyInstance.SetBaseOffsetExt(-200 / 304.8);//对每一项起点减去标高
            familyInstance.SetTopOffsetExt(2000 / 304.8);//设置底部偏移
            ...
            MessageBox.Show("新柱体已经生成！");

            uiDoc.ShowElements(familyInstance as Element);
